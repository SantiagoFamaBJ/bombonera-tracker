...
function parseFecha(raw: string): string | null {
  // Formatos: 27/1/2018, 27-1-2018, 2018-01-27
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      ...
      for (const line of lines) {
        ...
        // Saltar headers y filas de año (ej: "2018", "FECHA", etc.)
        if (!col0 || isNaN(Number(col0.replace(/\D/g, ''))) && !/\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4}/.test(col0)) continue
    ...
    <div className="min-h-screen bg-boca-dark p-4">
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="flex items-center gap-3 py-2">
          ...
          <h1 className="font-display text-boca-amarillo text-xl font-bold tracking-widest">
            IMPORTAR CSV
          </h1>
        </div>
        ...
        {!done && (
          <label className="block cursor-pointer">
            <div className="bg-boca-azul border-2 border-dashed border-boca-amarillo rounded-2xl p-8 text-center hover:border-yellow-300 transition-colors">
              <p className="text-4xl mb-2">📂</p>
              <p className="font-display text-boca-amarillo font-bold">SELECCIONAR CSV</p>
              <p className="text-blue-300 text-xs mt-1">o tocá para buscar el archivo</p>
            </div>
              ...
              {preview.map((p, i) => (
                <div
                  ...
                  className={`flex items-center gap-2 text-xs py-1.5 px-2 rounded-lg ${
                    p.error ? 'bg-red-900/40 text-red-300' : 'bg-blue-900/60 text-white'
                  }`}
                >
        ...
        {done && (
          <div className="bg-green-900/40 border border-green-600 rounded-2xl p-6 text-center space-y-3">
            ...
            <a
              href="/"
              className="block bg-boca-amarillo text-boca-dark py-3 rounded-xl font-display font-bold tracking-wide"
            >
              VER STATS
            </a>
          </div>
